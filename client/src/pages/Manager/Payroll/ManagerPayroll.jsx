...
export default function ManagerPayroll() {
  ...
  if (loading) {
    return (
      <ManagerLayout title="My Payroll" subtitle="View your salary details">
        <Loader />
      </ManagerLayout>
    );
  }
        ...
        {latest && (
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <DetailItem label="Late Deduction" value={fmtCurrency(latest.lateDeduction)} negative />
              <DetailItem label="Leave Deduction" value={fmtCurrency(latest.leaveDeduction)} negative />
            </div>
                  ...
                  {filtered.map((p) => {
                    ...
                    return (
                      <tr key={p._id} style={{ borderTop: "1px solid var(--color-border)" }}>
                        ...
                        <td className="px-5 py-3 text-right" style={{ color: "var(--color-text-secondary)" }}>
                          {fmtCurrency(p.grossEarnings)}
                        </td>
                        <td className="px-5 py-3 text-right" style={{ color: "#dc2626" }}>
                          -{fmtCurrency(p.totalDeductions)}
                        </td>
                        <td className="px-5 py-3 text-right font-bold" style={{ color: "var(--color-text-primary)" }}>
...
function DetailItem({ label, value, negative }) {
  return (
    <div>
      ...
      <p className="text-sm font-bold" style={{ color: negative ? "#dc2626" : "var(--color-text-primary)" }}>
