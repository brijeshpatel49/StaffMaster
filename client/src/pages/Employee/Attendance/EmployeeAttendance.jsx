...
const EmployeeAttendance = () => {
  ...
  const fetchMonthly = useCallback(async () => {
    ...
    try {
      const result = await apiFetch(`${API}/attendance/my?month=${month}&year=${year}`);
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (result?.data?.success) {
        ...
        setActionError(result?.data?.message || "Check-in failed");
      }
    } catch {
  ...
  return (
    <EmployeeLayout title="Attendance" subtitle="Track your daily attendance and work hours.">
      {/* ── Check-in/out Card ── */}
      <div
        style={{
          backgroundColor: "var(--color-card)",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "20px", flexWrap: "wrap" }}>
          ...
          {todayLoading ? (
            ...
            <p style={{ color: "var(--color-text-muted)", margin: 0, fontSize: "14px", fontWeight: 500 }}>
          ...
          {todayLoading ? null : !hasCheckedIn ? (
            ...
            <>
              ...
              >
                {actionLoading ? (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: "8px" }}>
                    <div style={{ width: "16px", height: "16px", border: "2px solid var(--color-border)", borderTop: "2px solid var(--color-text-primary)", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                    Checking out…
                  </span>
                ) : (
                  <>
      ...
      {showConfirm && (
        ...
        >
          ...
          >
            <h3 style={{ margin: "0 0 8px", color: "var(--color-text-primary)", fontSize: "18px" }}>
              Ready to check out?
            </h3>
      ...
      {summary && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "16px", marginBottom: "24px" }}>
          <StatCard icon={CheckCircle2} label="Present" value={summary.totalPresent} iconBg="var(--color-icon-green-bg)" iconColor="var(--color-icon-green)" />
          <StatCard icon={AlertTriangle} label="Late" value={summary.totalLate} iconBg="var(--color-icon-yellow-bg)" iconColor="var(--color-icon-yellow)" />
          <StatCard icon={Coffee} label="Half-day" value={summary.totalHalfDay} iconBg="var(--color-icon-purple-bg)" iconColor="var(--color-icon-purple)" />
          <StatCard icon={TrendingUp} label="Total Hours" value={`${summary.totalWorkHours}h`} iconBg="var(--color-icon-blue-bg)" iconColor="var(--color-icon-blue)" />
        </div>
      )}
                ...
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r._id}
                      style={{
                        backgroundColor: isToday ? "var(--color-accent-bg)" : "transparent",
                        transition: "background-color 0.15s",
                      }}
                      onMouseEnter={(e) => {
                        if (!isToday) e.currentTarget.style.backgroundColor = "var(--color-border-light)";
                      }}
                      onMouseLeave={(e) => {
                        if (!isToday) e.currentTarget.style.backgroundColor = "transparent";
                      }}
                    >
                      <td style={{ padding: "12px 16px", fontSize: "14px", fontWeight: 600, color: "var(--color-text-primary)", borderBottom: "1px solid var(--color-border)", whiteSpace: "nowrap" }}>
